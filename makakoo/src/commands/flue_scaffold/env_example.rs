...
fn channel_env(c: &ChannelSpec) -> Vec<(String, String)> {
    match c {
        ChannelSpec::Telegram { token_env, .. } => vec![
            (
                token_env.clone(),
                "Telegram bot token (from @BotFather)".into(),
            ),
            (
                "TELEGRAM_WEBHOOK_SECRET_TOKEN".into(),
                "Random string; set the same value on setWebhook".into(),
            ),
        ],
        ChannelSpec::Slack {
            token_env,
            ...
            (
                team_id_env.clone(),
                "Slack workspace team_id (T0123ABCD)".into(),
            ),
        ],
        ChannelSpec::Discord { token_env, .. } => {
            vec![(token_env.clone(), "Discord bot token".into())]
        }
        ChannelSpec::Webhook { secret_env, .. } => vec![(
            secret_env.clone(),
            "HMAC secret for inbound webhook signature verification".into(),
        )],
        ChannelSpec::Email { secret_env, .. } => vec![(
            secret_env.clone(),
            "Email credential JSON ({\"user\":\"…\",\"pass\":\"…\"})".into(),
        )],
        ChannelSpec::Voice {
...
fn trigger_env(t: &TriggerSpec) -> Vec<(String, String)> {
    match t {
        ...
        TriggerSpec::Webhook { secret_env, .. } => vec![(
            secret_env.clone(),
            "HMAC secret for trigger webhook signature verification".into(),
        )],
    }
}
