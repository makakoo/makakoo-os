#!/usr/bin/env python3
"""
Frozen Snapshot Memory — Harvey OS Core Memory Pattern.
...
Key constraint: System prompt gets snapshot at session start; mid-session writes
persist to disk but don't mutate the snapshot. Character limits (Memory 2200, User 1375).
"""
...
# ---------------------------------------------------------------------------
# MemoryStore — Frozen Snapshot Memory
# ---------------------------------------------------------------------------


def _get_harvey_home() -> Path:
    """Get HARVEY_HOME path, fallback to ~/.harvey."""
    HarveyHome = os.environ.get("HARVEY_HOME")
    if HarveyHome:
        return Path(HarveyHome)
    return Path.home() / ".harvey"
...
class MemoryStore:
    ...
    def load_from_disk(self) -> None:
        ...
        memory_dir = _get_harvey_home() / "frozen-memory"
    ...
    def _path_for(target: str) -> Path:
        """Return path for target memory file."""
        memory_dir = _get_harvey_home() / "frozen-memory"
        if target == "user":
