...
LOGSEQ_BRIDGE_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "logseq_bridge.py")
...
def _load_via_api(endpoint: str, method: str = "GET", data: dict = None) -> Optional[dict]:
    """Try loading data from Brain API."""
    try:
...
def _load_page_via_api(page_name: str) -> Optional[str]:
    """Load a page's content via Brain API."""
