...
Usage frequency reads real recall data from recall_stats when available,
falling back to the legacy access_count field.
"""
...
class MemoryScorer:
    ...
    def _recency_decay(self, memory: Dict) -> float:
        ...
        Uses proper exponential decay: e^(-(ln2/T) * age) where T = half_life_days.
        Unlike linear/sqrt, exponential never hits zero — old memories fade but survive.
        """
        last_update = (
    ...
    def _usage_frequency(self, memory: Dict) -> float:
        """
        Real recall frequency from recall_stats table when available.
        Falls back to legacy access_count field.
        Uses log scale (same as promoter) to avoid over-weighting high-frequency items.
        """
        # Try recall_stats first (populated by RecallTracker)
        ...
        if content_hash:
            try:
                ...
                with sqlite3.connect(tracker.db_path, check_same_thread=False) as conn:
                    conn.row_factory = sqlite3.Row
                    row = conn.execute(
                        "SELECT recall_count FROM recall_stats WHERE content_hash = ?",
                        (content_hash,),
                    ).fetchone()
                if row and row["recall_count"] > 0:
                    return min(1.0, math.log1p(row["recall_count"]) / math.log1p(20))
            except Exception:
                pass
