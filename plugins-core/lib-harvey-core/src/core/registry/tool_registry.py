#!/usr/bin/env python3
"""
Tool Registry — Self-registering tool registry singleton for Harvey OS.

Each tool file calls ``registry.register()`` at module level to declare its
schema, handler, toolset membership, and availability check.

Import chain (circular-import safe):
    core/registry/tool_registry.py  (no imports from tool files)
           ^
    core/tools/*.py  (import from tool_registry at module level)
           ^
    Any module that dispatches tools

Key concepts:
- Tools self-register via registry.register() at import time
- ToolRegistry is a singleton with schema + handler + metadata
- Tools can declare availability via check_fn (runtime requirements check)
- Registry provides dispatch, schema retrieval, and toolset queries

Source pattern: hermes-agent/tools/registry.py (275 lines)
"""
...
class ToolEntry:
    ...
    Uses __slots__ for memory efficiency with many tool entries.
    """
...
class ToolRegistry:
    ...
    Provides:
    - register()/deregister() for tool management
    - get_definitions() for OpenAI-format tool schemas
    - dispatch() for executing tools by name
    - is_toolset_available() for runtime availability checks
    """
    ...
    def register(
        ...
        Warns on name collision (same name, different toolset).
        Stores check_fn for toolset-level availability checking.
        """
    ...
    def deregister(self, name: str) -> None:
        ...
        same toolset. Used for dynamic tool discovery cleanup.
        """
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        are included. Availability is cached per check_fn to avoid
        redundant calls.
        """
        result = []
    ...
    def dispatch(self, name: str, args: dict, **kwargs) -> Any:
        ...
        - Async handlers are bridged automatically via asyncio
        ...
        try:
            if entry.is_async:
                ...
                # For now, return the coroutine for the caller to await
                # In a real async context, this would be awaited
                return asyncio.ensure_future(run_in_loop())
