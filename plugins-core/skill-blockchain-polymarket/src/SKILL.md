---
name: polymarket
description: Use when you need to query Polymarket prediction market data — search markets, get prices, orderbooks, and price history. Read-only via public REST APIs, no API key needed.
version: 1.0.0
...
| Phrase | Effect |
|--------|--------|
| `polymarket odds for [question]` | Searches Polymarket for markets matching the specified question and returns current Yes/No prices as percentages |
| `what are the odds on Polymarket` | Activates Polymarket search mode to find relevant prediction markets |
| `prediction market prices for [topic]` | Queries Polymarket for all markets related to the specified topic |
| `check Polymarket for [event]` | Searches for Polymarket markets asking about the specified event |
| `show me Polymarket orderbook for [market]` | Retrieves detailed orderbook data for a specific market |
| `Polymarket price history of [question]` | Fetches historical price data for the specified market |
