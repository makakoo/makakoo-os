...
---
<!--
FIXME (Harvey provenance, 2026-04-11):
This skill was mirrored from garrytan/gstack. It contains CLI-specific paths:
  - `~/.claude/skills/gstack/bin/...` helper invocations
  - `.claude/skills/gstack/...` fallbacks
  - `~/.gstack/` shared state directory
These paths are intentionally LEFT AS-IS so the skill remains runnable when
the user has gstack installed alongside Harvey. Harvey should eventually abstract
these through a CLI-agnostic helper (e.g. `$HARVEY_SKILLS_BIN`) so these skills
work even when gstack is not installed. Until then:
  - `~/.claude/skills/gstack/bin/` -> FIXME: CLI-specific path, Harvey should abstract via a helper
  - `~/.gstack/`                   -> gstack's own state dir; shared with upstream gstack when both installed
-->
